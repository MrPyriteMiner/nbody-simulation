...
from spread_reader import read_spread
...
def make_bodies_using_maker(param1=0, param2=0, param3=0, param4=0):
    positions, velocities = read_spread()
    body_list = []
...
if __name__ == "__main__":
    ...
    bodi_es = make_bodies_using_maker(2)
    ...
    while time < total_time:
        ...
        if stepcount % (int(1/timescale/5000/ts_factor)+1) == 0:
            draw(verse)
        if timer.time() > last_time + 0.1:
            last_time = time
            ...
            print(str(math.floor(time * 1000000/total_time)/10000) + "%")
    end = timer.time()
    ...
    plt.plot(helper_array, kinetic_energy_array, label='kin')
    plt.plot(helper_array, potential_energy_array, label='pot')
    plt.plot(helper_array, total_energy_array, label='tot')
    leg = plt.legend(loc='upper left')
    plt.show()
    plt.plot(helper_array, momentum_array, label='mom')
    leg = plt.legend(loc='upper left')
    plt.show()
    plt.plot(helper_array, total_energy_array, label='tot')
    leg = plt.legend(loc='upper left')
    plt.show()
    plt.plot(helper_array, total_delta_energy, label='tot_adj')
    leg = plt.legend(loc='upper left')
    plt.show()
